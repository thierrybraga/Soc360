...
def fetch_nvd_cves(vendor, last_updated):
    ...
    try:
        ...
        for item in data.get("vulnerabilities", []):
            ...
            if published_date and (published_date <= last_updated or int(published_date[:4]) < 2020):
                continue
        ...
        for cve in vendor_cves:
            ...
            if not exists:
                ...
                if existing and (
                    ...
                    # Atualiza CVE e limpa campo risks
                    cve["risks"] = ""  # ou None, conforme o tipo do campo no banco
                    update_cve(cve)
                    logging.info(f"CVE atualizada e campo 'risks' limpo: {cve['cve_id']}")

        if new_cves:
            populate_database({"vulnerabilities": new_cves})
            logging.info(f"Novas CVEs inseridas para o vendor {vendor}: {len(new_cves)}")

        logging.info("Aguardando 15 segundos para evitar limite de rate da API do NVD...")
        time.sleep(15)
...
def schedule_update():
    ...
    scheduler.add_job(update_vulnerabilities, 'interval', hours=4)
    scheduler.start()
    logging.info("Agendamento a cada 4 horas configurado com sucesso.")
