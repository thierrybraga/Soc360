...
def settings():
    ...
    # Aviso persistente para admin que deve alterar senha
    if current_user.is_authenticated and getattr(current_user, 'is_admin', False) and (getattr(current_user, 'force_password_reset', False) or current_user.username.lower() == 'admin'):
        flash('Sua conta administrador está em estado inseguro. Por favor, altere a senha imediatamente.', 'warning')
